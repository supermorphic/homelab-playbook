...
CANDIDATE_VALIDATION_PATH = (
    REPOSITORY_ROOT / "scripts" / "ci" / "candidate_validation.py"
)
...
class TemporaryGitRepository:
    def __init__(self) -> None:
        ...
        self.git("config", "user.name", "Candidate Validation Test")
        self.git("config", "user.email", "candidate-validation@example.invalid")
...
class CandidateValidationTests(unittest.TestCase):
    ...
    def test_pr_range_rejects_committed_trailing_whitespace(self) -> None:
        ...
        result = self.run_validation(CI_BASE_SHA=self.base, CI_HEAD_SHA=head)
    ...
    def test_full_ci_rejects_untracked_working_tree_secret(self) -> None:
        ...
        result = self.run_validation(FULL_SECRET_SCAN="1")
